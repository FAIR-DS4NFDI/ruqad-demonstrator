...
def collect_records_created_after(manager: KadiManager, cut_of_date: datetime.datetime)->list(int):
    """
    Iterates page-wise over the responses of the Kadi API until records are reached that are older
    than the given cut_of_date.

    Paremters
    ---------
    manager: KadiManager, KadiManager instance used to connect to the Kadi API
    cut_of_date: datetime, Records that were created after this date are included in the returned
                 list

    Returns
    -------
        list(int), list of IDs of records that were created after the given cut_of_date
    """
    record_ids = []
    done = False
    for response in _generate_pages(manager):
        for el in response["items"]:
            if cut_of_date > datetime.fromisoformat(el["created_at"]):
                done = True
                break
